...
output "model_gateway_connection_name" {
  description = "Foundry ModelGateway connection name (null when disabled)."
  value       = var.enable_model_gateway_connection ? var.model_gateway_connection_name : null
}

output "model_gateway_target" {
  description = "Base URL of the gateway the Foundry connection points at (null when disabled)."
  value       = var.enable_model_gateway_connection ? var.model_gateway_target : null
}

output "agent_model_references" {
  description = "Values to pass as the agent `model` field: <connection>/<deployment>. Only populated for static discovery; with dynamic discovery the deployment list comes from the gateway at runtime."
  value = var.enable_model_gateway_connection ? [
    for m in var.model_gateway_static_models : "${var.model_gateway_connection_name}/${m.name}"
  ] : []
}
